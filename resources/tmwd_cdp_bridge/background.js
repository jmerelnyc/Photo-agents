...
function scheduleProbe() {
  ...
  chrome.alarms.create('PHA-WS-probe', { delayInMinutes: 0.083 }); // ~5s
}

function scheduleKeepalive() {
  // Keep SW alive while WS is connected (~25s, under 30s SW timeout)
  chrome.alarms.create('PHA-WS-keepalive', { delayInMinutes: 0.4 }); // ~24s
}
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  ...
  if (alarm.name === 'PHA-WS-keepalive') {
  ...
  if (alarm.name === 'PHA-WS-probe') {
    if (ws && ws.readyState <= 1) return; // Already connected/connecting
    if (await isServerAlive()) {
      console.log('[PHA-WS] Server detected, connecting...');
      connectWS();
    } else {
...
async function handleWsExec(data) {
  ...
  console.log('[PHA-WS] Exec request', data.id, 'on tab', tabId);
  ...
  try {
    ...
    try {
      ...
      if (res === null || res === undefined) {
        console.log('[PHA-WS] executeScript returned null/undefined, treating as CSP issue');
        res = { ok: false, error: { name: 'Error', message: 'executeScript returned null (possible CSP or context issue)', stack: '' }, csp: true };
      }
    } catch (e) {
      console.log('[PHA-WS] scripting.executeScript failed:', e.message);
    ...
    if (res && !res.ok && res.csp) {
      console.log('[PHA-WS] CDP fallback for tab', tabId);
...
function connectWS() {
  ...
  ws = null;
  console.log('[PHA-WS] Connecting to', WS_URL);
  try {
    ws = new WebSocket(WS_URL);
  } catch (e) {
    console.error('[PHA-WS] Constructor error:', e);
    ws = null;
    scheduleProbe();
    return;
  }
  ws.onopen = async () => {
    console.log('[PHA-WS] Connected!');
    ...
    console.log('[PHA-WS] Sent ext_ready with', tabs.length, 'tabs');
  };
  ws.onmessage = async (event) => {
    try {
      ...
      console.error('[PHA-WS] message parse error', e);
    }
  };
  ws.onclose = () => {
    console.log('[PHA-WS] Disconnected');
    ws = null;
    scheduleProbe();
  };
  ws.onerror = (e) => {
    console.error('[PHA-WS] Error:', e);
