"""LLM provider router for Photo Agents.

This module owns:

* The credential loader (``reload_credentials`` / ``_load_credentials``).
* SSE / JSON parsers for both the Anthropic ``messages`` API and the
  OpenAI ``chat/completions`` and ``responses`` APIs.
* The session classes the runtime dispatches to by keyword:

  - :class:`ClaudeSession` / :class:`LLMSession`         (relay-style protocol prompt)
  - :class:`NativeClaudeSession` / :class:`NativeOAISession` (provider-native tool calls)
  - :class:`MixinSession`                                  (round-robin failover)
  - :class:`ToolClient` / :class:`NativeToolClient`        (handler-friendly facade)

Public class names are deliberately preserved because the runtime's
session-selection logic in ``photoagents.cli.runtime`` checks them by
substring (``"native"`` / ``"claude"`` / ``"oai"`` / ``"mixin"``).
"""

from __future__ import annotations

import base64  # noqa: F401  (some user-side scripts import this transitively)
import importlib
import json
import os
import re
...
_creds_mtime: int | None = None


def _load_credentials() -> dict:
    """Return the user's credentials as a flat ``{name: cfg}`` dict.

    Tries ``credentials.py`` (preferred — supports comments and Python
    expressions) then ``credentials.json`` next to this package.
    """
    global _creds_path
    try:
        import credentials  # type: ignore[import-not-found]
        importlib.reload(credentials)
        _creds_path = credentials.__file__
        return {k: v for k, v in vars(credentials).items() if not k.startswith("_")}
    except ImportError:
        pass

    _creds_path = path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "..", "..", "credentials.json"
    )
    if not os.path.exists(path):
        raise Exception(
            "[ERROR] credentials.py or credentials.json not found. "
            "Copy photoagents/config/keys_template.py to credentials.py at the "
            "project root and fill in your provider keys."
        )
...
def reload_credentials():
    """Re-read credentials from disk if the file's mtime has changed.

    Returns ``(creds_dict, changed_bool)``. The first call always returns
    ``changed=True``.
    """
    global _creds_mtime
    mt = os.stat(_creds_path).st_mtime_ns if _creds_path else -1
    if mt == _creds_mtime:
        return globals().get("creds", {}), False
    creds = _load_credentials()
    _creds_mtime = os.stat(_creds_path).st_mtime_ns
    print(f"[Info] Loaded credentials from {_creds_path}")
    globals().update(creds=creds)
    if creds.get("langfuse_config"):
        try:
            from photoagents.integrations import langfuse_tracing  # noqa: F401
        except Exception:
            pass
    return creds, True


# Backwards-compat alias for any code that still calls the old name.
reload_mykeys = reload_credentials


def __getattr__(name):  # PEP 562 — lazy "creds" attribute
    if name == "creds":
        return reload_credentials()[0]
    if name == "mykeys":
        return reload_credentials()[0]
    raise AttributeError(f"module 'photoagents.llm.router' has no attribute {name}")
...
def compress_history_tags(messages, keep_recent: int = 10, max_len: int = 800,
                         force: bool = False):
    """Trim ``<thinking>`` / ``<tool_use>`` / ``<tool_result>`` payloads in
    older turns to claw back tokens.

    Runs at most every 5th call unless ``force=True``.
    """
    ...
    pats = {tag: re.compile(rf"(<{tag}>)([\s\S]*?)(</{tag}>)")
    ...
    print(f"[Cut] {before} -> {after}")
    return messages


def _sanitize_leading_user_msg(msg):
    """Rewrite ``tool_result`` blocks in a leading user message into plain text.

    History is normalized to Claude-style content blocks (``content`` is a
    list of blocks). Without this, an orphaned ``tool_result`` at the head
    of a trimmed history confuses the provider.
    """
    msg = dict(msg)
...
# stdout swallow guard (windowless launchers can lose stdout mid-print)
_oldprint = print
...
def trim_messages_history(history, context_win):
    ...
    if cost > context_win * 3:
        # Trimming busts caches anyway, so be aggressive.
...
def _parse_claude_sse(resp_lines):
    """Parse an Anthropic SSE stream. Yields text chunks; returns the assembled
    ...
    for line in resp_lines:
        ...
        try:
            ...
            print(f"[SSE] JSON parse error: {exc}, line: {data_str[:200]}")
            continue
        ...
        if evt_type == "message_start":
            ...
            if block.get("type") == "text":
                ...
                current_block = {"type": "tool_use", "id": block.get("id", ""),
...
def _try_parse_tool_args(raw):
    """Parse a tool-args string. Splits concatenated JSON objects like
    ``{..}{..}`` if needed. Returns a list of dicts."""
    if not raw:
        return [{}]
    try:
...
def _parse_openai_sse(resp_lines, api_mode: str = "chat_completions"):
    """Parse an OpenAI SSE stream (either ``chat/completions`` or ``responses``).
    Yields text chunks; returns a list of content blocks compatible with the
    Anthropic-style block layout used elsewhere in the runtime."""
    content_text = ""

    if api_mode == "responses":
        seen_delta = False
        fc_buf: dict[int, dict] = {}
        ...
        for line in resp_lines:
            ...
            if etype == "response.output_text.delta":
                ...
                if item.get("type") == "function_call":
                    ...
                    fc_buf[idx] = {"id": item.get("call_id", item.get("id", "")),
                                   "name": item.get("name", ""), "args": ""}
                    current_fc_idx = idx
...
def _parse_openai_json(data, api_mode: str = "chat_completions"):
    blocks = []
    if api_mode == "responses":
...
# ---------------------------------------------------------------------------
# Anthropic-via-OpenAI cache hints
# ---------------------------------------------------------------------------

def _stamp_oai_cache_markers(messages, model):
    """If the model is an Anthropic family routed via an OpenAI-compatible
    relay, add cache_control markers to the last 2 user messages."""
    ml = model.lower()
...
# ---------------------------------------------------------------------------
# Streaming HTTP wrapper with retry
# ---------------------------------------------------------------------------
    ...
    for attempt in range(sess.max_retries + 1):
        ...
        try:
            ...
            if attempt < sess.max_retries:
                ...
                print(f"[LLM Retry] {type(exc).__name__}, retry in {d:.1f}s "
                      f"({attempt + 1}/{sess.max_retries + 1})")
                yield err
                time.sleep(d)
                continue
            ...
            err = (f"\n\n[!!! Stream interrupted {type(exc).__name__}: {exc} !!!]"
                   if streamed else f"!!!Error: {type(exc).__name__}: {exc}")
            yield err
            return [{"type": "text", "text": err}]


# ---------------------------------------------------------------------------
# OpenAI-style outbound
# ---------------------------------------------------------------------------
...
def _openai_stream(sess, messages):
    ...
    if api_mode == "responses":
        ...
        if sess.max_tokens:
            payload["max_completion_tokens" if ml.startswith(("gpt-5", "o1", "o2", "o3", "o4"))
                    else "max_tokens"] = sess.max_tokens
        if sess.reasoning_effort:
    ...
    parse_fn = ((lambda r: _parse_openai_sse(r.iter_lines(), api_mode))
                if sess.stream else (lambda r: _parse_openai_json(r.json(), api_mode)))
    return (yield from _stream_with_retry(sess, url, headers, payload, parse_fn))


def _prepare_oai_tools(tools, api_mode: str = "chat_completions"):
    if api_mode == "responses":
        resp_tools = []
        for t in tools:
...
def _to_responses_input(messages):
    ...
    for msg in messages:
        ...
        if role == "tool":
            cid = msg.get("tool_call_id") or (pending.pop(0) if pending
                                              else f"call_{uuid.uuid4().hex[:8]}")
            result.append({"type": "function_call_output", "call_id": cid,
        ...
        if not parts:
            parts = [{"type": text_type,
                      "text": str(content) if not isinstance(content, list) else "[empty]"}]
        ...
        for tc in (msg.get("tool_calls") or []):
            ...
            pending.append(cid)
            result.append({"type": "function_call", "call_id": cid,
                           "name": f.get("name", ""),
...
def _msgs_claude2oai(messages):
    """Convert a Claude-style content-block message list into OpenAI shape."""
    result = []
    for msg in messages:
...
class BaseSession:
    def __init__(self, cfg):
        ...
        self.context_win = cfg.get("context_win", 28_000)
        self.history: list = []
        ...
        def _enum(key, valid):
            ...
            if not v or v in valid:
                return v
            print(f"[WARN] Invalid {key} {v!r}, ignored.")
            return None

        self.reasoning_effort = _enum("reasoning_effort",
    ...
    def _apply_claude_thinking(self, payload):
        ...
        if self.reasoning_effort:
            effort = {"low": "low", "medium": "medium", "high": "high",
                      "xhigh": "max"}.get(self.reasoning_effort)
            if effort:
                payload["output_config"] = {"effort": effort}
            else:
                print(f"[WARN] reasoning_effort {self.reasoning_effort!r} is unsupported "
                      "for Claude output_config.effort, ignored.")
...
def _ensure_thinking_blocks(messages, model):
    """DeepSeek requires assistant turns to have a thinking block. Stub one in
    if missing."""
...
class ClaudeSession(BaseSession):
    def raw_ask(self, messages):
        ...
        parse_fn = ((lambda r: _parse_claude_sse(r.iter_lines()))
                    if self.stream else (lambda r: _parse_claude_json(r.json())))
        return (yield from _stream_with_retry(self, url, headers, payload, parse_fn))

    def make_messages(self, raw_list):
        msgs = _drop_unsigned_thinking([{"role": m["role"], "content": list(m["content"])}
                                        for m in raw_list])
        user_idxs = [i for i, m in enumerate(msgs) if m["role"] == "user"]
        for idx in user_idxs[-2:]:
            msgs[idx]["content"][-1] = dict(msgs[idx]["content"][-1],
                                            cache_control={"type": "ephemeral"})
        return msgs
...
def _fix_messages(messages):
    """Ensure the message list satisfies Claude's API constraints: alternating
    roles, paired tool_use / tool_result blocks."""
    if not messages:
        return messages
    ...
    for m in messages:
        if fixed and m["role"] == fixed[-1]["role"]:
            fixed[-1] = {**fixed[-1],
                         "content": _wrap(fixed[-1]["content"]) + [{"type": "text", "text": "\n"}] + _wrap(m["content"])}
            continue
        if fixed and fixed[-1]["role"] == "assistant" and m["role"] == "user":
            uses = [b.get("id") for b in fixed[-1].get("content", [])
                    if isinstance(b, dict) and b.get("type") == "tool_use" and b.get("id")]
            has = {b.get("tool_use_id") for b in _wrap(m["content"])
                   if isinstance(b, dict) and b.get("type") == "tool_result"}
            miss = [uid for uid in uses if uid not in has]
            if miss:
                m = {**m, "content": [{"type": "tool_result", "tool_use_id": uid,
                                        "content": "(error)"} for uid in miss] + _wrap(m["content"])}
            orphan = has - set(uses)
            if orphan:
                m = {**m, "content": [
                    {"type": "text", "text": str(b.get("content", ""))}
                    if isinstance(b, dict) and b.get("type") == "tool_result" and b.get("tool_use_id") in orphan
                    else b for b in _wrap(m["content"])
                ]}
        fixed.append(m)
...
class NativeClaudeSession(BaseSession):
    ...
    def raw_ask(self, messages):
        ...
        if self.max_tokens is None:
            self.max_tokens = 8192
        model = self.model
        beta_parts = ["claude-code-20250219", "interleaved-thinking-2025-05-14",
        ...
        for idx in user_idxs[-2:]:
            ...
            messages[idx]["content"][-1] = dict(messages[idx]["content"][-1],
                                                cache_control={"type": "ephemeral"})
    ...
    def ask(self, msg):
        ...
        with self.lock:
            ...
            trim_messages_history(self.history, self.context_win)
            messages = [{"role": m["role"], "content": list(m["content"])}
...
# ---------------------------------------------------------------------------
# Mock response objects (for handler-friendly facade)
# ---------------------------------------------------------------------------
...
class MockToolCall:
    def __init__(self, name, args, id: str = ""):
...
class MockResponse:
    def __init__(self, thinking, content, tool_calls, raw, stop_reason: str = "end_turn"):
    ...
    def __repr__(self):
        return (f"<MockResponse thinking={bool(self.thinking)}, "
                f"content='{self.content}', tools={bool(self.tool_calls)}>")


# ---------------------------------------------------------------------------
# Tool clients (the public surface used by the runtime)
# ---------------------------------------------------------------------------
...
class ToolClient:
    ...
    def _prepare_tool_instruction(self, tools):
        ...
        instruction = (
            ...
            "2. **Summarize**: Output a minimal one-line (<30 words) physical snapshot in "
            "`<summary>`: new info from last tool result + current tool call intent. This goes "
            "into long-term working memory. Must contain real information, no filler.\n"
            "3. **Act**: If you need to call tools, output one or more **<tool_use> blocks** "
            "after your reply, then stop.\n"
        )
        instruction += (
            f'\nFormat: ```<tool_use>{{"name": "tool_name", "arguments": {{...}}}}'
    ...
    def _build_protocol_prompt(self, messages, tools):
        system_content = next((m["content"] for m in messages
    ...
    def _parse_mixed_response(self, text):
        ...
        if think_match:
            thinking = think_match.group(1).strip()
            remaining_text = re.sub(r"<think(?:ing)?>(.*?)</think(?:ing)?>", "",
        ...
        if not tool_calls:
            ...
            if "<tool_use>" in remaining_text:
                weak = remaining_text.split("<tool_use>")[-1].strip().strip("><")
                json_str = weak if weak.endswith("}") else ""
                if (json_str == "" and "```" in weak
                        and weak.split("```")[0].strip().endswith("}")):
                    json_str = weak.split("```")[0].strip()
                if json_str:
                    json_strs.append(json_str)
                remaining_text = remaining_text.replace("<tool_use>" + weak, "")
            ...
            for json_str in json_strs:
                try:
                    ...
                    args = (data.get("arguments") or data.get("args")
                            or data.get("params") or data.get("parameters"))
                    if args is None:
                        args = data
                    if func_name:
...
def _parse_text_tool_calls(content):
    """Fallback: extract tool calls from plain text when the model did not use
    a native tool_use block."""
    tcs = []
    # Try a top-level JSON array: [{"type":"tool_use", ...}]
    _jp = next((p for p in ['[{"type":"tool_use"', '[{"type": "tool_use"']
                if p in content), None)
    if _jp and content.endswith("}]"):
        try:
            idx = content.index(_jp)
            raw = json.loads(content[idx:])
            tcs = [MockToolCall(b["name"], b.get("input", {}), id=b.get("id", ""))
                   for b in raw if b.get("type") == "tool_use"]
            return tcs, content[:idx].strip()
        except Exception:
            pass
    # Try XML tags: <tool_call>{"name":..., "arguments":...}</tool_call>
    _xp = (r"<(?:tool_use|tool_call)>"
           r"((?:(?!<(?:tool_use|tool_call)>).){15,}?)"
           r"</(?:tool_use|tool_call)>")
...
def _write_llm_log(label, content):
    log_dir = os.path.join(os.path.expanduser("~/.photoagents"), "temp", "model_responses")
...
class MixinSession:
    """Multi-session failover with spring-back to the primary."""
    ...
    def __init__(self, all_sessions, cfg):
        ...
        self._sessions = [
            ...
            next(s.backend for s in all_sessions if not isinstance(s, dict) and s.backend.name == i)
            for i in cfg.get("llm_nos", [])
        ]
        is_native = lambda s: "Native" in s.__class__.__name__
        groups = {is_native(s) for s in self._sessions}
        assert len(groups) == 1, (
            "MixinSession: sessions must be all Native or all non-Native, got "
            f"{[type(s).__name__ for s in self._sessions]}"
        )
    ...
    _BROADCAST_ATTRS = frozenset({"system", "tools", "temperature", "max_tokens",
                                  "reasoning_effort", "history"})

    def __setattr__(self, name, value):
        if name in self._BROADCAST_ATTRS:
            for s in self._sessions:
                v = openai_tools_to_claude(value) if (name == "tools"
                                                      and type(s) is NativeClaudeSession) else value
                setattr(s, name, v)
        else:
    ...
    def _raw_ask(self, *args, **kwargs):
        ...
        for attempt in range(self._retries + 1):
            ...
            try:
                ...
                return_val = exc.value or []
            is_err = test_error(last_chunk)
            if not is_err:
                if attempt > 0:
                    self._cur_idx = idx
                    self._switched_at = time.time()
                elif (isinstance(last_chunk, str) and "[!!! Stream interrupted" in last_chunk
                      and n > 1):
                    self._cur_idx = (idx + 1) % n
                    self._switched_at = time.time()
                    print(f"[MixinSession] Partial failure, next call -> "
            ...
            if nxt == base:  # full round failed; wait before next round
                rnd = (attempt + 1) // n
                delay = min(30, self._base_delay * (1.5 ** rnd))
                print(f"[MixinSession] {last_chunk[:80]}, round {rnd} exhausted, "
                      f"retry in {delay:.1f}s")
                time.sleep(delay)
            else:
                print(f"[MixinSession] {last_chunk[:80]}, retry {attempt + 1}/{self._retries} "
                      f"(s{idx}->s{nxt})")
...
class NativeToolClient:
    ...
    def set_system(self, extra_system):
        combined = (f"{extra_system}\n\n{self._thinking_prompt()}"
                    if extra_system else self._thinking_prompt())
    ...
    def chat(self, messages, tools=None):
        ...
        for tr in tool_results:
            ...
            if tool_use_id:
                tool_result_blocks.append({"type": "tool_result",
                                           "tool_use_id": tool_use_id,
                                           "content": content})
            else:
                combined_content = ([{"type": "text",
                                      "text": f"<tool_result>{content}</tool_result>"}]
                                    + combined_content)
        for tid in self._pending_tool_ids:
            if tid not in tr_id_set:
                tool_result_blocks.append({"type": "tool_result",
                                           "tool_use_id": tid, "content": ""})
