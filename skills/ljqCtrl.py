"""
CRITICAL: never import pyautogui in this toolchain — it pollutes win32api and
creates conflicts with the manual hooks below.

ljqCtrl quick reference:
- dpi_scale: float (Logical = Physical * dpi_scale)
- Click(x, y): use physical coordinates (matching screenshots)
...
dpi_scale = 1
try:
    from PIL import ImageGrab, Image, ImageEnhance, ImageFilter, ImageDraw  # noqa: F401
    import cv2
except Exception:
    pass

_hdc = ctypes.windll.user32.GetDC(0)
swidth = ctypes.windll.gdi32.GetDeviceCaps(_hdc, 118)   # DESKTOPHORZRES (physical)
sheight = ctypes.windll.gdi32.GetDeviceCaps(_hdc, 117)  # DESKTOPVERTRES
ctypes.windll.user32.ReleaseDC(0, _hdc)
cwidth = win32api.GetSystemMetrics(win32con.SM_CXSCREEN)   # logical
...
def FindBlock(fn, wrect=None, verbose=0, threshold=0.8):
    ...
    tsh, tsw = T.shape[:2]
    if verbose:
        print('T.shape:', T.shape, '\t', 'B.shape:', B.shape)
