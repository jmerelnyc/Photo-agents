...
# WinAPI types — defined explicitly so the script works on 64-bit Python.
...
def scan_memory(pid, pattern, context_size=256, mode='auto', llm_mode=False):
    ...
    if not h_proc:
        # OpenProcess failed: might be a system process or higher integrity level.
    ...
    # Range covering the 64-bit user space.
    max_addr = 0x7FFFFFFFFFFF

    while curr_addr < max_addr:
        # Cast keeps the pointer type correct on 64-bit.
        res = k32.VirtualQueryEx(h_proc, ctypes.cast(curr_addr, LPCVOID), ctypes.byref(mbi), ctypes.sizeof(mbi))
        if res == 0:
            break

        # MEM_COMMIT = 0x1000; 0xEE covers the common readable protection flags.
                ...
                for match in rules.match(data=data):
                    for inst in match.strings:
                        ...
                        if llm_mode:
                            ...
                            # Expand context based on context_size to capture the full KEY+SALT block.
