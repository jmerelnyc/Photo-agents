"""L4 session log processor — compress sessions and extract their history.

Format A (JSON): kept as-is.
Format B (raw): strip the system prompt and the assistant echo.
"""
import os
import re
from datetime import datetime

L4_DIR = os.path.expanduser('~/.photoagents/sessions')
...
def _detect_format(text):
    """Detect format A (json) vs format B (raw) by checking what follows the first Prompt marker."""
...
def _parse_sections(text):
    """Split the text into (type, marker_line, body) tuples."""
...
def _compress_raw(text):
    """Format B: strip the system prompt (Prompt -> USER) and the assistant echo (ASSISTANT -> Response)."""
...
def compress_session(src, dst_dir=None):
    """Compress ``model_responses_xxx.txt`` -> ``MMDD_HHMM-MMDD_HHMM.txt``.

    Returns ``(dst, stats)`` on success, or ``(None, reason)`` on skip/failure.
    """
...
def _parse_history_block(raw):
    """Parse a ``<history>`` block into a list of ``[USER]...``/``[Agent]...`` lines."""
    lines = [l.strip() for l in raw.split('\n') if l.strip()]
    parsed = [l for l in lines if l.startswith('[USER]') or l.startswith('[Agent]')]
    if len(parsed) >= 2:
        return parsed
    # JSON-encoded variant where lines are separated by literal ``\n``.
    joined = raw.strip()
    if '\\n[USER]' in joined or '\\n[Agent]' in joined:
...
def _merge_history_blocks(all_blocks):
    """Merge sliding-window history blocks into a single deduplicated list."""
    if not all_blocks:
        return []
...
def extract_history(src, session_name=None):
    """Extract ``[USER]``/``[Agent]`` history from a session file."""
    with open(src, 'r', encoding='utf-8', errors='replace') as f:
        text = f.read()
...
def format_history_block(session_name, history_lines):
    """Format history lines into the all_histories.txt block format."""
    sep = '=' * 60
...
def _existing_sessions(l4_dir):
    """Read session names that already appear in all_histories.txt."""
...
def batch_process(src, l4_dir=None, dry_run=True):
    """Batch compress + extract history + archive. ``dry_run=True`` is the safe default."""
    ...
    import time
    cutoff = time.time() - 7200  # skip files modified within the last 2 hours

    # Phase 1: Compress + Extract (into the temp dir)
    for fp in raw_files:
    ...
    for f, e in errors[:5]:
        print(f"  ERR  {f}: {e}")
    if results:
        print(f"  Range: {results[0][0]} -> {results[-1][0]}")
    ...
    # Phase 3: Archive into per-month zips
    ...
    for fname, reason in skipped:
        if 'recent' in reason:
            continue  # active session is still being written
...
# ── CLI ──
RAW_DIR = os.path.expanduser('~/.photoagents/sessions/raw')
