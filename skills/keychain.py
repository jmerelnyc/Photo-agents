"""Keychain: save a key into a file, then ``keys.set("name", file="path")``;
use ``keys.name.use()`` to retrieve it (use the value, do not print it)."""
import getpass
import hashlib
import json
import os
...
_PATH = pathlib.Path.home() / ".photoagents" / "keychain.enc"
...
_MASK = hashlib.sha256(f"{_user}@photoagents_keychain".encode()).digest()
...
class _Keys:
    ...
    def set(self, k, v=None, *, file=None):
        if file:
            v = pathlib.Path(file).read_text().strip()
        self._d[k] = v
