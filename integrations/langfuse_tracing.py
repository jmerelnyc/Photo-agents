...
Self-activates on import if ``langfuse_config`` exists in the user's
credentials. Hooks are applied via monkey-patch so the core runtime files
stay completely untouched:

- :func:`photoagents.core.loop.run_agent_session` -> outer ``agent.task`` trace
- :func:`photoagents.llm.router._write_llm_log`   -> generation span
- :class:`photoagents.core.loop.BaseHandler` tool callbacks -> per-tool span
"""
...
try:
    from photoagents.llm.router import _load_credentials
    _cfg = _load_credentials().get("langfuse_config")
...
if _lf:
    from photoagents.core import loop as _loop
    from photoagents.llm import router as _router

    _tls = threading.local()

    # ----- LLM generation span ---------------------------------------------
    _orig_log = _router._write_llm_log

    def _patched_log(label, content):
        try:
            if label == "Prompt":
                _tls.gen = _lf.start_observation(name="llm.chat", as_type="generation",
                                                 input=content[:20_000])
                _tls.usage = None
            elif label == "Response" and getattr(_tls, "gen", None) is not None:
                _tls.gen.update(output=content[:20_000],
    ...
    _router._write_llm_log = _patched_log

    # ----- Token-usage extraction from streaming SSE ------------------------
    ...
    _router._parse_claude_sse = _wrap_parser(_router._parse_claude_sse)
    _router._parse_openai_sse = _wrap_parser(_router._parse_openai_sse)

    # ----- Per-tool span ----------------------------------------------------
    _orig_before = _loop.BaseHandler.tool_before_callback
    _orig_after = _loop.BaseHandler.tool_after_callback
    ...
    _loop.BaseHandler.tool_before_callback = _patched_before
    _loop.BaseHandler.tool_after_callback = _patched_after

    # ----- Outer agent.task span -------------------------------------------
    _orig_loop = _loop.run_agent_session

    def _patched_loop(client, system_prompt, user_input, handler, tools_schema, *a, **kw):
        try:
            cm = _lf.start_as_current_observation(name="agent.task", as_type="agent",
    ...
    _loop.run_agent_session = _patched_loop
    # Backwards-compat alias also gets the patched version.
    _loop.agent_runner_loop = _patched_loop
    for _m in list(sys.modules.values()):
        if _m and getattr(_m, "run_agent_session", None) is _orig_loop:
            try:
                setattr(_m, "run_agent_session", _patched_loop)
            except Exception:
                pass
