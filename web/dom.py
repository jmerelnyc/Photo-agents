...
except ImportError: print("[Error] BeautifulSoup4 is not installed. Ask the agent to install beautifulsoup4 before using web tools.")
...
function createEnhancedDOMCopy() {
  ...
  function cloneNode(sourceNode, keep=false) {
    ...
    try { if (sourceNode.matches && sourceNode.matches(':-webkit-autofill')) { clone.setAttribute('data-autofilled', 'true'); if (!sourceNode.value) clone.setAttribute('value', '[protected - see driver SOP autofill section to extract]'); } } catch(e) {}
    ...
    // aria-hidden + not visible = truly hidden (e.g. mobile menus), drop even if it has children
...
function analyzeNode(node, pPathType='main') {
    // Skip non-element and leaf nodes
    ...
    // Collect child geometry and sort by area
    ...
    // Detect partition vs. overlay layout
  ...
  function handlePartitionContainer(childrenInfo, pathType) {
    ...
    if (hasMainElement) {
      ...
      for (let i = 1; i < childrenInfo.length; i++) {
        ...
        if (child.node.textContent.trim().length > 200) isSecondary = true;  // P3: keep when there is real text content
  ...
  function handleOverlayContainer(childrenInfo, pathType) {
    // elementFromPoint ground truth: ask the browser which element is on top visually
    ...
    console.log('children sorted by zIndex:', sorted);
...
// Hoist top 1-2 deep fixed dialogs to body level so overlay detection sees them
const _fc = [...domCopy.querySelectorAll('*')].filter(el => {
  ...
  .slice(0, 2);
_fc.forEach(el => { const r = getNodeInfo(el).rect; console.log('[dom] Hoisted fixed dialog:', el.tagName + (el.id ? '#'+el.id : '') + (el.className ? '.'+String(el.className).split(' ')[0] : ''), Math.round(r.width)+'x'+Math.round(r.height), Math.round(100*r.width*r.height/viewportArea)+'%'); el.parentNode.removeChild(el); domCopy.appendChild(el); });
...
js_findMainList = r'''function findMainList(startElement = null) {
        ...
        // Global scan: collect candidate containers, ranked by l1 + l2*0.1 (l2 = grandchild count, captures multi-layer structures like tables)
        const candidates = [];
        ...
        // For each container, find candidate groups and score them
        let allCandidates = [];
        ...
        // Dedup: drop candidates that overlap a higher-scoring one by more than 50%
        const kept = [];
        ...
        function describeResult(container, items, selector, score) {
            if(container&&!container.id)container.id='_pha'+(window._lci=(window._lci||0)+1);
    ...
    function findTopGroups(container, limit) {
        ...
        // Tally tag and class frequencies
        ...
        // Return the top-N highest-scoring groups
    ...
    function scoreContainer(container, items) {
        ...
        // Too few visible items -> low score
        if (visibleItems < 3) return 0;
        // Guard against outliers: total item area cannot exceed container
        totalItemArea = Math.min(totalItemArea, containerArea * 0.98);
        const areaRatio = totalItemArea / containerArea;
        // 3. Component scores - linear interpolation rather than step functions
        // 3.2 Area-ratio score - up to 40 points, smooth curve
        // Sigmoid for a smoother distribution
        const areaScore = 40 / (1 + Math.exp(-12 * (areaRatio - 0.4)));

        // 3.3 Uniformity score - up to 20 points, smooth curve
        ...
        if (itemAreas.length >= 3) {
            ...
            // Exponential decay: lower cv -> higher score
        ...
        // 3.4 Container size score - up to 15 points, smooth curve
        const viewportArea = window.innerWidth * window.innerHeight;
        ...
        let layoutScore = 0;
        if (items.length >= 3) {
            // Bucket coordinates and count rows/cols
            ...
            // Single row or single column scores full marks; otherwise grade the grid
        ...
        // Total - still roughly a 100-point scale
...
def get_main_block(driver, extra_js="", text_only=False):
    ...
    if text_only:
        page = re.sub(r' {2,}', ' ', page)            # collapse runs of spaces
        page = re.sub(r'^ +', '', page, flags=re.M)   # strip leading spaces per line
        page = re.sub(r'(\n\s*){3,}', '\n\n', page)   # 3+ blank lines -> single blank line
        return page.strip()
    return page
...
def find_changed_elements(before_html, after_html):
    ...
    # Change boundary: elements whose parent is not itself in the changed set
...
def smart_truncate(soup, budget, _depth=0):
    """Truncate the soup in-place so its serialized size approaches `budget`.

    Strategy: drill through single-child chains to find a branching point;
    if the top three children can absorb the overflow, share the cut
    proportionally; otherwise drop trailing children.
    """
    CUT_THRESHOLD = 8000  # below this, just trim the tail; above, recurse to find a branch
    indent = '  ' * _depth
    def cut(ele, keep):
    ...
    # === single child: recurse through it ===
    if len(kids) == 1:
    ...
    # Check whether the top 3 can absorb the overflow
    ...
    if top_total < over:
        # === top 3 cannot absorb it: drop children from the end ===
        removed = 0
        removed_count = 0
        ...
        return soup
    # === top 2-3 share the cut proportionally ===
    # Drop kids smaller than 10% of the largest so the big ones absorb everything
    ...
    # Plan all allocations first (and log them)
    actions = []
    for i in tops:
        c, l = kids[i]
...
# Backwards-compatible alias for callers that expect simplify_html / get_simplified
simplify_html = optimize_html_for_tokens
get_simplified = get_html
...
def execute_js_rich(script, driver, no_monitor=False):
    ...
    if response.get('newTabs'): rr['newTabs'] = response['newTabs']
        ...
        if new_sids:
            ...
            rr['newTabs'] = newTabs
            rr['suggestion'] = "Page reloaded; the listed new tabs connected during execution."
    ...
    if not reloaded and len(newTabs) == 0:
        try:
            ...
            if change_count == 0 and not transients and len(newTabs) == 0:
                ...
                rr['suggestion'] = "No visible page change"
        except:
