...
class TMWebDriver:
        ...
        if not session or not session.is_active():
            ...
            if not session or not session.is_active():
                ...
                if alive_sessions:
                    ...
                    print(f"Session {session_id} not connected; switching to most recent active session: {session.id}")
                    session_id = self.default_session_id = session.id
                if not session or not session.is_active():
                    raise ValueError(f"Session id {session_id} is not connected")
    ...
    def _remote_cmd(self, cmd):
        try: return requests.post(self.remote, headers={"Content-Type": "application/json"}, json=cmd).json()
            ...
            raise ConnectionError("TMWebDriver master is not running; see the driver SOP for how to start the master.")
    ...
    def set_session(self, url_pattern: str) -> bool:
        ...
        if not matched: return print(f"Warning: no session found whose URL contains '{url_pattern}'")
        if len(matched) > 1: print(f"Warning: multiple sessions match URL '{url_pattern}'; using the first one")
        self.default_session_id, info = matched[0]
        print(f"Default session set: {self.default_session_id}: {info['url']}")
