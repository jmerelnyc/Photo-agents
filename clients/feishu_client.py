...
from photoagents.clients.client_common import format_restore
from photoagents.clients.resume_cmd import handle_frontend_command as handle_continue_frontend, reset_conversation
from photoagents.llm.router import creds
...
_TRUNC_TAIL = 300  # number of trailing characters to keep when falling back to truncation
...
def _display_text(text):
    ...
    return "Model output was truncated or empty" + (f"\n...{tail}" if tail else "")
...
APP_ID = str(creds.get("fs_app_id", "") or "").strip()
APP_SECRET = str(creds.get("fs_app_secret", "") or "").strip()
ALLOWED_USERS = _to_allowed_set(creds.get("fs_allowed_users", []))
...
def _send_raw(receive_id, payload, msg_type, rtype):
    ...
    print(f"send failed: {r.code}, {r.msg}")
    return None
...
def _patch_card(message_id, card_json):
    ...
    if not r.success():
        print(f"[ERROR] patch_card failed: {r.code}, {r.msg}")
    return r.success()
...
def _send_local_file(receive_id, file_path, receive_id_type="open_id"):
    if not os.path.isfile(file_path):
        send_message(receive_id, f"File does not exist: {file_path}", receive_id_type=receive_id_type)
        return False
    ext = os.path.splitext(file_path)[1].lower()
    if ext in _IMAGE_EXTS:
        ...
        if file_key:
            ...
            return True
    send_message(receive_id, f"Failed to send file: {os.path.basename(file_path)}", receive_id_type=receive_id_type)
    return False
...
def _build_step_detail(resp, tool_calls):
    """Compose per-step expanded detail from LLM response + tool_calls (pure)."""
    parts = []
...
class _TaskCard:
    """Feishu task card: a single card kept up-to-date via patch; one collapsible
    panel per step (header shows summary, expand for details)."""
    _DETAIL_LIMIT = 8000
    ...
    def _step_panel(self, idx, summary, detail):
        detail = detail or "_(no output)_"
    ...
    def step(self, summary, detail=""):
        ...
        self.status = f"Working - Turn {len(self.steps)}"
        self._push()

    def done(self, text):
        self.status = "Done"
        self.final = text or "_(no text output)_"
        self._push()

    def fail(self, msg):
        self.status = f"Failed: {msg}"
        self._push()


def _make_task_hook(card, done_event, on_final):
    """Feishu task hook: patch the card on each turn; on completion call on_final(raw)
    to send any attachments."""
    def hook(ctx):
        try:
...
def handle_message(data):
    ...
    if not PUBLIC_ACCESS and open_id not in ALLOWED_USERS:
        print(f"unauthorized user: {open_id}")
        return
    user_input, image_paths = _build_user_message(message)
    if not user_input:
        if chat_id:
            send_message(chat_id, f"Unsupported Feishu message type: {message.message_type}", receive_id_type="chat_id")
        else:
            send_message(open_id, f"Unsupported Feishu message type: {message.message_type}")
        return
    print(f"received message [{open_id}] ({message.message_type}, {len(image_paths)} images): {user_input[:200]}")
...
def handle_command(open_id, cmd, chat_id=None):
    ...
    if op == "/stop":
        ...
        agent.abort()
        _send_cmd_response("Stopping...")
    elif op == "/new":
        _send_cmd_response(reset_conversation(agent))
    elif op == "/help":
        _send_cmd_response("Commands:\n/stop - stop the current task\n/status - show status\n/llm - list models\n/llm [n] - switch to model N\n/restore - restore previous conversation history\n/continue - list restorable sessions\n/continue [n] - restore session N\n/new - start a new conversation and clear context\n/help - show help")
    elif op == "/status":
        llm = agent.get_llm_name() if agent.llmclient else "not configured"
        _send_cmd_response(f"Status: {'running' if agent.is_running else 'idle'}\nLLM: [{agent.llm_no}] {llm}")
    elif op == "/llm":
        if not agent.llmclient:
            return _send_cmd_response("No LLM configured")
        if len(parts) > 1:
            try:
                agent.next_llm(int(parts[1]))
                return _send_cmd_response(f"Switched to [{agent.llm_no}] {agent.get_llm_name()}")
            except Exception:
                return _send_cmd_response(f"Usage: /llm <0-{len(agent.list_llms()) - 1}>")
        ...
        try:
            ...
            if err:
                return _send_cmd_response(err)
            restored, fname, count = restored_info
            agent.history.extend(restored)
            agent.abort()
            _send_cmd_response(f"Restored {count} conversation rounds\nSource: {fname}\n(Context only restored; please send a new question to continue)")
        except Exception as e:
            _send_cmd_response(f"Restore failed: {e}")
    elif op == "/continue" or cmd.startswith("/continue"):
        _send_cmd_response(handle_continue_frontend(agent, cmd))
    else:
        _send_cmd_response(f"Unknown command: {cmd}")


def main():
    global client
    if not APP_ID or not APP_SECRET:
        print("Error: please set fs_app_id and fs_app_secret in credentials.py or credentials.json")
        sys.exit(1)
    ...
    print("=" * 50 + "\nFeishu Agent started (long-connection mode)\n" + f"App ID: {APP_ID}\nWaiting for messages...\n" + "=" * 50)
    cli.start()
