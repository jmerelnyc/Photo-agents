...
TELEGRAM_MENU_COMMANDS = (
    ...
    ("llm", "List models; /llm n switches to the Nth model"),
)
...
def _native_first_user_line(prompt_text):
    ...
    if "### Current user message" in text:
        text = text.split("### Current user message", 1)[-1].strip()
    return text
...
def require_runtime(agent, label, **required):
    ...
    if missing:
        print(f"[{label}] ERROR: please set {', '.join(missing)} in credentials.py or credentials.json")
        sys.exit(1)
    if agent.llmclient is None:
        print(f"[{label}] ERROR: no usable LLM backend found in credentials.py or credentials.json")
        sys.exit(1)
...
class AgentChatMixin:
    ...
    async def handle_command(self, chat_id, cmd, **ctx):
        ...
        if op == "/stop":
            ...
            if state:
                state["running"] = False
            self.agent.abort()
            return await self.send_text(chat_id, "Stopping...", **ctx)
        if op == "/status":
            llm = self.agent.get_llm_name() if self.agent.llmclient else "not configured"
            return await self.send_text(chat_id, f"Status: {'running' if self.agent.is_running else 'idle'}\nLLM: [{self.agent.llm_no}] {llm}", **ctx)
        if op == "/llm":
            if not self.agent.llmclient:
                return await self.send_text(chat_id, "No LLM configured", **ctx)
            if len(parts) > 1:
                try:
                    self.agent.next_llm(int(parts[1]))
                    return await self.send_text(chat_id, f"Switched to [{self.agent.llm_no}] {self.agent.get_llm_name()}", **ctx)
                except Exception:
                    return await self.send_text(chat_id, f"Usage: /llm <0-{len(self.agent.list_llms()) - 1}>", **ctx)
        ...
        if op == "/restore":
            try:
                ...
                return await self.send_text(chat_id, f"Restored {count} conversation rounds\nSource: {fname}\n(Context only restored; please send a new question to continue)", **ctx)
            except Exception as e:
                return await self.send_text(chat_id, f"Restore failed: {e}", **ctx)
        if op == "/continue":
    ...
    async def run_agent(self, chat_id, text, **ctx):
        ...
        try:
            await self.send_text(chat_id, "Thinking...", **ctx)
            ...
            while state["running"]:
                try:
                    ...
                    if self.agent.is_running and time.time() - last_ping > self.ping_interval:
                        await self.send_text(chat_id, "Still working, please wait...", **ctx)
                        last_ping = time.time()
                    continue
                if "done" in item:
            ...
            if not state["running"]:
                await self.send_text(chat_id, "Stopped", **ctx)
        except Exception as e:
            import traceback
            print(f"[{self.label}] run_agent error: {e}")
            traceback.print_exc()
            await self.send_text(chat_id, f"Error: {e}", **ctx)
        finally:
            self.user_tasks.pop(chat_id, None)


from photoagents.cli.runtime import PhotoAgentsRuntime as _GA
from photoagents.clients.resume_cmd import handle_frontend_command as _handle_continue_frontend, install as _install_continue, reset_conversation as _reset_conversation
