...
class WxBotClient:
    ...
    def login_qr(self, poll_interval=2):
        ...
        while True:
            ...
            st = s.get('status', '')
            if st != last: print(f'  status: {st}'); last = st
            if st == 'confirmed':
                self.token, self.bot_id = s.get('bot_token', ''), s.get('ilink_bot_id', '')
                self._save(login_time=time.strftime('%Y-%m-%d %H:%M:%S'))
                print(f'[QR login] success! bot_id={self.bot_id}')
                return s
            if st == 'expired': raise RuntimeError('QR code expired')
    ...
    def run_loop(self, on_message, poll_timeout=30):
        print(f'[Bot] listening... (bot_id={self.bot_id})')
        seen = set()
        while True:
            try:
                for msg in self.get_updates(poll_timeout):
                    ...
                    except Exception as e: print(f'[Bot] callback error: {e}')
            except KeyboardInterrupt: print('[Bot] exit'); break
            except Exception as e: print(f'[Bot] error: {e}, retry in 5s'); time.sleep(5)
...
def _clean(t):
    ...
    t = re.sub(r'^\s*[\U0001F300-\U0001FAFF]?\s*[A-Za-z_][A-Za-z0-9_]*\(.*$', '', t, flags=re.M)
    for p in _TAG_PATS:
...
def on_message(bot, msg):
    ...
    if media_paths:
        text = (text + '\n' if text else '') + '\n'.join(f'[user sent file: {p}]' for p in media_paths)
    print(f'[WX] received: {text[:80]}', file=sys.__stdout__)

    # Commands
    if text in ('/stop', '/abort'):
        agent.abort()
        bot.send_text(uid, 'Stopped', context_token=ctx)
        return
    if text.startswith('/llm'):
        ...
        if len(args) > 1:
            try:
                ...
                bot.send_text(uid, f'Switched to [{agent.llm_no}] {agent.get_llm_name()}', context_token=ctx)
            except (ValueError, IndexError):
                bot.send_text(uid, f'Usage: /llm <0-{len(agent.list_llms())-1}>', context_token=ctx)
        else:
        ...
        try:
            while True:
                ...
                if len(done) > sent:
                    ...
                    if _send(merged):
                        sent = len(done)
        except queue.Empty: result = '[timeout]'
...
if __name__ == '__main__':
    ...
    _logf = open(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'temp', 'wechat_client.log'), 'a', encoding='utf-8', buffering=1)
    ...
    print(f'WeChat bot started (bot_id={bot.bot_id})', file=sys.__stdout__)
