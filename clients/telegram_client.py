...
try:
    ...
    sys.exit(1)
from photoagents.clients.client_common import (
    FILE_HINT,
    HELP_TEXT,
...
from photoagents.clients.resume_cmd import handle_frontend_command, reset_conversation
from photoagents.llm.router import creds

agent = GeneraticAgent()
agent.verbose = False
agent.inc_out = True
ALLOWED = set(creds.get('tg_allowed_users', []))
...
_ASK_CANCEL_PROMPT = "Selection cancelled. Please send your next instruction directly."
...
def _extract_ask_user_event(ctx):
    ...
    if not candidates:
        return None
    question = str(data.get("question") or "Please choose the next step:").strip() or "Please choose the next step:"
...
def _normalize_ask_menu_event(stored):
    if isinstance(stored, dict):
        ...
        return {
            "question": str(stored.get("question") or "Please choose the next step:").strip() or "Please choose the next step:",
            "candidates": [str(candidate).strip() for candidate in candidates if str(candidate).strip()],
        }
    if isinstance(stored, (list, tuple)):
        return {
            "question": "Please choose the next step:",
...
def _render_ask_user_result(event, selected=None, cancelled=False):
    question = str(event.get("question") or "Please choose the next step:").strip() or "Please choose the next step:"
    ...
    lines.append("")
    if cancelled:
        lines.append(f"Cancelled: {_ASK_CANCEL_LABEL}")
    elif selected:
...
async def _stream(dq, msg):
    ...
    try:
        ...
        await stream.finish_with_notice("Stopped")
    except RetryAfter as exc:
        ...
        if stream.session is not None and stream.session._is_retrying():
            return
        try:
            await stream.finish_with_notice(f"Output failed: {exc}")
        except RetryAfter as retry_exc:
...
async def handle_ask_callback(update, ctx):
    ...
    if not menu_id:
        return await query.answer("Invalid menu")
    event = _normalize_ask_menu_event(_ask_menu_store.get(menu_id))
    if event is None:
        await query.answer("Menu expired")
...
async def handle_photo(update, ctx):
    ...
    prompt = f"[TIPS] received {kind} temp/{fpath}\n{caption}" if caption else f"[TIPS] received {kind} temp/{fpath}, please wait for the next instruction"
    dq = agent.put_task(prompt, source="telegram")
    task = asyncio.create_task(_stream(dq, update.message))
    ctx.user_data['stream_task'] = task
...
async def handle_command(update, ctx):
    ...
    if op == '/status':
        ...
        return await update.message.reply_text(f"Status: {'running' if agent.is_running else 'idle'}\nLLM: [{agent.llm_no}] {llm}")
    ...
    if op == '/restore':
        ...
        try:
            ...
            return await update.message.reply_text(f"Restored {count} conversation rounds\nSource: {fname}\n(only context restored; please type a new question to continue)")
        except Exception as e:
            return await update.message.reply_text(f"Restore failed: {e}")
    if op == '/continue':
...
if __name__ == '__main__':
    ...
    if not ALLOWED:
        print('[Telegram] ERROR: tg_allowed_users in credentials.py is empty or missing. Set it to avoid unauthorized access.')
        sys.exit(1)
    require_runtime(agent, "Telegram", tg_bot_token=creds.get("tg_bot_token"))
    redirect_log(__file__, "telegram_client.log", "Telegram", ALLOWED)
    ...
    while True:
        try:
            ...
            app = (ApplicationBuilder().token(creds['tg_bot_token'])
