"""
Single-file desktop frontend - PySide6 chat panel + floating button.
Dependencies: pip install PySide6
Optional: pip install markdown  (Markdown rendering)
Usage: python clients/desktop_app.py
"""
...
class FloatingButton(QWidget):
    ...
    def _toggle(self):
        ...
        if now - self._last_toggle_ms < 500:
            return
...
def _build_prompt_with_uploads(prompt: str, files: list) -> tuple:
    ...
    attachment_chunks = ["\n\n[User uploaded attachments - files are saved on local disk and can be read with the file_read tool]"]
    display_attachments = []
    ...
    for f in files:
        ...
        if mime.startswith("image/"):
            ...
            attachment_chunks.append(
                ...
                + ("\n[Content truncated; use file_read to read the full content]" if len(text) > MAX_INLINE_CHARS else "")
            )
    ...
    if file_names:
        parts.append(f"{len(file_names)} file(s) ({', '.join(file_names)})")
...
class _MsgRow(QWidget):
    """A single message row - flat layout with avatar, inspired by ChatGPT-style UIs."""
...
class ChatPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        self._stack.addWidget(self._build_sop_page())
    ...
    def _build_titlebar(self) -> QWidget:
        ...
        self._search_input.setPlaceholderText("Search current chat and history...")
    ...
    def _build_input_area(self) -> QWidget:
        ...
        self._input.setPlaceholderText("Send a message to the assistant... Enter to send, Shift+Enter for newline")
    ...
    def _do_switch_to(self, idx: int):
        ...
        self._add_system_notice(f"Switched to {name}; conversation context preserved")
        self._refresh_model_rows_style()
    ...
    def _check_backend(self, idx: int, backend):
        ...
        try:
            ...
            # Some backends return generators (e.g. NativeClaudeSession.ask)
        ...
        if hasattr(backend, 'raw_msgs') and backend.raw_msgs:
            backend.raw_msgs = [m for m in backend.raw_msgs if m.get("prompt") != "hello"]
    ...
    def _handle_send(self):
        ...
        prompt = text or "Please analyze the attachments I uploaded."
    ...
    def _refresh_history(self):
        ...
        for s in reversed(self._history[-20:]):
            ...
            item = QListWidgetItem(f"  {s.get('title','Untitled')}   ({n} msgs)")
    ...
    def _update_token_usage(self):
        ...
        if in_tokens == 0 and out_tokens == 0:
            ...
            self._token_lbl.setText(f"|   Context tokens: in {in_tokens}  out {out_tokens}")
    ...
    def _load_sop(self, item):
        ...
        try:
            ...
            self._sop_viewer.setPlainText(f"Read failed: {e}")
    ...
    def _do_toggle_auto(self):
        ...
        lbl = "Pause autonomous mode" if self.autonomous_enabled else "Enable autonomous mode (auto-trigger after 30 min idle)"
        self._auto_btn.setText(lbl)

    def _do_trigger_auto(self):
        self.inject_message(
            "[AUTO] User triggered autonomous mode. Please read the automation SOP, pick a valuable task, and execute it."
        )
...
def main():
    ...
    app.setApplicationName("Photo Agents")
    ...
    if agent.llmclient is None:
        QMessageBox.critical(
            None,
            "No LLM configured",
            "No usable LLM endpoint was found in credentials.py.\nThe app will run without an LLM.",
        )
    else:
    ...
    scr = QApplication.primaryScreen().availableGeometry()
    print(f"[Photo Agents] started")
    print(f"  Screen size: {scr.width()}x{scr.height()}")
    print(f"  Floating button: ({button.x()}, {button.y()})")
    print(f"  Chat panel: ({panel.x()}, {panel.y()})")
    print(f"  Click the glowing button at bottom-right to reopen the panel after closing it")
    ...
    def idle_check():
        ...
        if idle > 1800:
            ...
            panel.inject_message(
                "[AUTO] User has been away for over 30 minutes. As an autonomous agent, please read the automation SOP and execute an automatic task."
            )
