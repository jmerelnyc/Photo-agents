"""Desktop companion with skin system - cross-platform with true transparency"""
...
def _normalize_bubble_text(text):
    ...
    lines = text.replace('\r\n', '\n').replace('\r', '\n').split('\n')
    if lines:
        turn_match = re.match(r'^\s*[\U0001F300-\U0001FAFF]?\s*Turn\s+(\d+)\s*$', lines[0], flags=re.IGNORECASE)
        if turn_match:
            rest = '\n'.join(line.strip() for line in lines[1:] if line.strip())
            return f"Turn {turn_match.group(1)}: {rest}" if rest else f"Turn {turn_match.group(1)}:"
    return text.replace('Turn', 'Turn').strip()
...
# ============================================================================
class CompanionBase:
    """Shared logic for Mac and Windows companion implementations."""
        ...
        class Handler(BaseHTTPRequestHandler):
            def do_GET(self):
                ...
                if 'state' in params:
                    ...
                    companion.set_state_safe(state)
                    self.send_response(200)
                    self.end_headers()
                    self.wfile.write(b'ok')
                elif 'msg' in params:
                    msg = params['msg'][0]
                    companion.show_toast_safe(msg)
                    self.send_response(200)
                    self.end_headers()
                    self.wfile.write(b'ok')
                else:
            ...
            def do_POST(self):
                ...
                if body:
                    companion.show_toast_safe(body)
                    self.send_response(200)
                    self.end_headers()
                    self.wfile.write(b'ok')
                else:
...
if sys.platform == 'darwin':
    ...
    class MacCompanion(CompanionBase):
        def __init__(self, skin_name=None):
            ...
            # Stick across spaces (stays in fixed screen position)
            ...
            class DraggableImageView(NSView):
                ...
                def initWithFrame_(self, frame):
                    ...
                    # Overlay view for toast (always on top); non-opaque so the image stays visible
                ...
                def rightMouseDown_(self, event):
                    ...
                    companion = self.window().delegate()

                    for skin_name in companion.available_skins:
                        item = NSMenuItem.alloc().initWithTitle_action_keyEquivalent_(
                            skin_name,
                            'changeSkin:',
                            ''
                        )
                        item.setTarget_(companion)
            ...
            print(f"macOS Companion started at ({x_pos}, {y_pos})")
        ...
        def show_toast(self, message):
            """Show toast message above companion"""
            ...
            companion_frame = self.window.frame()
            anchor_x = companion_frame.origin.x + self.display_width * 0.75
            anchor_y = companion_frame.origin.y + self.display_height * 1.65
...
# ============================================================================
    ...
    if sys.platform.startswith('win'):
        ...
        class WinCompanion(CompanionBase):
            def __init__(self, skin_name=None):
                self.root = tk.Tk()
                self.root.wm_attributes('-topmost', True)
                self.is_windows = sys.platform.startswith('win')
                self.platform_name = 'Windows' if self.is_windows else 'Linux'
                self.companion_bg_color = '#F0F0F0' if self.is_windows else 'black'
                ...
                if self.is_windows:
                    self.root.wm_attributes('-transparentcolor', self.companion_bg_color)
                self.root.config(bg=self.companion_bg_color)

                self.label = tk.Label(self.root, bg=self.companion_bg_color, bd=0)
                self.label.pack()
                ...
                print(f"{self.platform_name} Companion started at ({x_pos}, {y_pos})")
                print(f"  Animations: {', '.join(self.animations.keys())}")
            ...
            def show_toast(self, message):
                ...
                companion_x = self.root.winfo_x()
                companion_y = self.root.winfo_y()
                anchor_x = companion_x + int(self.display_width * 0.75)
                anchor_y = companion_y
                toast_x = anchor_x - tail_x
        ...
        class _LinuxCompanionLabel(QLabel):
            ...
            def mousePressEvent(self, event):
                if event.button() == Qt.LeftButton:
                    self.drag_offset = event.globalPosition().toPoint() - self.companion.window.frameGeometry().topLeft()
                    event.accept()
                    return
                if event.button() == Qt.RightButton:
                    self.companion._show_context_menu(event.globalPosition().toPoint())
                    event.accept()
                    return
            ...
            def mouseMoveEvent(self, event):
                if self.drag_offset is not None and (event.buttons() & Qt.LeftButton):
                    self.companion.window.move(event.globalPosition().toPoint() - self.drag_offset)
                    self.companion._reposition_toast()
                    event.accept()
                    return
        ...
        class LinuxCompanion(CompanionBase):
            def __init__(self, skin_name=None):
                ...
                self.label = _LinuxCompanionLabel(self)
                ...
                print(f"Linux PySide6 Companion started at ({x_pos}, {y_pos})")
                print(f"  Animations: {', '.join(self.animations.keys())}")
            ...
            def _compute_toast_geometry(self, bubble_width, bubble_height, tail_x, tail_y):
                companion_pos = self.window.frameGeometry().topLeft()
                anchor_x = companion_pos.x() + int(self.display_width * 0.75)
                anchor_y = companion_pos.y() + int(self.display_height * 0.15)
...
if __name__ == '__main__':
    ...
    try:
        ...
        _s.close()
        print(f'Companion already running on port {PORT}, exiting.')
        sys.exit(0)
    except ConnectionRefusedError:
        pass

    if sys.platform == 'darwin':
        companion = MacCompanion('vita')
        companion.run()
    elif sys.platform.startswith('win'):
        companion = WinCompanion('vita')
    else:
        companion = LinuxCompanion('vita')
        companion.run()
