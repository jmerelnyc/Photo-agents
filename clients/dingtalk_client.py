...
from photoagents.clients.client_common import AgentChatMixin, ensure_single_instance, public_access, redirect_log, require_runtime, split_text
...
CLIENT_ID = str(creds.get("dingtalk_client_id", "") or "").strip()
CLIENT_SECRET = str(creds.get("dingtalk_client_secret", "") or "").strip()
ALLOWED = {str(x).strip() for x in creds.get("dingtalk_allowed_users", []) if str(x).strip()}
USER_TASKS = {}
...
class DingTalkApp(AgentChatMixin):
    ...
    async def start(self):
        ...
        while True:
            ...
            # Sessions that lived >=60s are treated as healthy -> reset backoff
...
if __name__ == "__main__":
    ...
    redirect_log(__file__, "dingtalk_client.log", "DingTalk", ALLOWED)
