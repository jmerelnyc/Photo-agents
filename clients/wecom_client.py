...
from photoagents.clients.client_common import (AgentChatMixin, FILE_HINT, build_done_text, clean_reply,
...
# --- Config ---------------------------------------------------------
BOT_ID    = str(creds.get("wecom_bot_id", "") or "").strip()
SECRET    = str(creds.get("wecom_secret", "") or "").strip()
WELCOME   = str(creds.get("wecom_welcome_message", "") or "").strip()
ALLOWED   = {str(x).strip() for x in creds.get("wecom_allowed_users", []) if str(x).strip()}
...
class WeComApp(AgentChatMixin):
    ...
    async def send_media(self, chat_id, file_path):
        ...
        try:
            ...
            await self.send_text(chat_id, f"{os.path.basename(file_path)} (send failed: {e})")
    ...
    async def run_agent(self, chat_id, text, **_):
        ...
        def _on_turn(ctx):
            """Turn-end callback injected into agent. ctx = locals() from handler."""
            try:
                ...
                if tools:
                    parts.append(f"Tools: {', '.join(_fmt_tool(tc) for tc in tools[:3])}")
        ...
        try:
            ...
            if result.get("raw") is not None:
                ...
                label = result.get("summary") or f'{len(result["raw"])} chars'
                _tprint(f"[{_ts()}] Done ({chat_id}) - {label}")
            elif not state["running"]:
                _tprint(f"[{_ts()}] Stopped ({chat_id})")
                await self.send_text(chat_id, "Stopped")
            else:
                _tprint(f"[{_ts()}] Abnormal exit ({chat_id})")
                await self.send_text(chat_id, "Agent exited unexpectedly. Please retry.")
        except Exception as e:
            traceback.print_exc()
            await self.send_text(chat_id, f"Error: {e}")
        finally:
    ...
    async def on_text(self, frame):
        ...
        if not content:
            return
        _tprint(f"[{_ts()}] msg {sender_id}: {content}")
        if content.startswith("/"):
            _tprint(f"[{_ts()}] command {content} from {sender_id}")
    ...
    async def _on_media(self, frame, key, icon):
        ...
        try:
            ...
            label = "an image" if key == "image" else f"a file {os.path.basename(path)}"
            asyncio.create_task(self.run_agent(chat_id, f"[user sent {label}, saved to: {path}]"))
        except Exception as e:
            print(f"[WeCom] on_{key} error: {e}")
            await self.send_text(chat_id, f"{key} processing failed: {e}")

    async def on_image(self, frame):
        await self._on_media(frame, "image", "[image]")

    async def on_file(self, frame):
        await self._on_media(frame, "file", "[file]")
    ...
    async def on_authenticated(self, *_): _tprint("[WeCom] authenticated, waiting for messages...\n")
    ...
    def _terminal_loop(self):
        ...
        while True:
            ...
            if not cmd:
                continue
            if cmd == "help":
                _tprint("  status        - show status")
                _tprint("  stop [user]   - stop a task (specify user when multiple are active)")
                _tprint("  exit          - exit the process")
            elif cmd == "status":
                _tprint(f"[{_ts()}] received {self._stats['received']} | completed {self._stats['completed']} | active {len(self.user_tasks)}")
                for uid, st in self.user_tasks.items():
                    _tprint(f"  - {uid}: running={st.get('running')}")
                _tprint(f"  Agent running: {self.agent.is_running} | allowed: {self._allowed or 'all'}")
            elif cmd.startswith("stop"):
                parts = cmd.split(None, 1)
                tasks = self.user_tasks
                if not tasks:
                    _tprint("  no active tasks")
                elif len(parts) > 1:
                    uid = parts[1]
                    if uid in tasks:
                        tasks[uid]["running"] = False
                        _tprint(f"  stopped {uid}")
                    else:
                        _tprint(f"  not found: {uid}")
                elif len(tasks) == 1:
                    uid = next(iter(tasks))
                    tasks[uid]["running"] = False
                    _tprint(f"  stopped {uid}")
                else:
                    _tprint("  multiple tasks; please specify: stop <user_id>")
                    for uid in tasks:
                        _tprint(f"  - {uid}")
            elif cmd == "exit":
                _tprint(f"[{_ts()}] exiting...")
                os._exit(0)
            else:
                _tprint("  available commands: help | status | stop | exit")
...
if __name__ == "__main__":
    ...
    redirect_log(__file__, "wecom_client.log", "WeCom", ALLOWED)
    _tprint("\n===========================================")
    _tprint("  WeCom Agent  (long-connection mode)")
    _tprint(f"  port lock: {PORT} | allowed users: {ALLOWED or 'all'}")
    _tprint("===========================================")
    _tprint("  terminal commands:  help | status | stop | exit")
