...
APP_ID = str(creds.get("qq_app_id", "") or "").strip()
APP_SECRET = str(creds.get("qq_app_secret", "") or "").strip()
ALLOWED = {str(x).strip() for x in creds.get("qq_allowed_users", []) if str(x).strip()}
...
if __name__ == "__main__":
    ...
    redirect_log(__file__, "qq_client.log", "QQ", ALLOWED)
